...
class InterviewListFragmentAppointing : Fragment() {
    ...
    var allTheData_APPOINTING: MutableList<InterviewInfo> = mutableListOf()
            ...
            request.create(Interview::class.java)
                ...
                .subscribe({
                    ...
                    for (i in 0..data.length() - 1) {
                        ...
                        var distanceToDeadlineStr="已过期"
                        var distanceToDeadline=(acceptDeadline-nowTime)
                        var anhour=1000*60*60
                        if(distanceToDeadline>0){
                            if(distanceToDeadline>anhour){
                                var hour:Int=((distanceToDeadline/anhour).toInt())
                                distanceToDeadlineStr=hour.toString()+"時間後、取り消す"
                            }else{
                                var aMinute=1000*60
                                var minute:Int=((distanceToDeadline/aMinute).toInt())
                                distanceToDeadlineStr=minute.toString()+"分間後、取り消す"
                        ...
                        if (item.has("appointedStartTime")) {
                            ...
                            if (startTime - nowTime > 0) {
                                ...
                                if (startTime - nowTime - distanceADay < 0) {
                                    appointedStartflag = "今日"
                                } else if (dateDistance == 1) {
                                    appointedStartflag = "明日"
                                } else if (dateDistance == 2) {
                                    appointedStartflag = "后日"
                                }
                            }
    ...
    fun appendDateToList(
        ...
        if (adapter == null) {
            //适配器
            adapter = InterviewListAdapter(recycler, list, "承認待ち") { item ->
                //跳转
